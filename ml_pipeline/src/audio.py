...
def load_audio(path: str, sample_rate: int) -> torch.Tensor:
    # Use soundfile directly to bypass torchaudio torchcodec errors
    wav_np, sr = sf.read(path)
    
    # Handle stereo vs mono
    if len(wav_np.shape) > 1:
        wav_np = wav_np.mean(axis=1) # average channels
        
    wav = torch.from_numpy(wav_np).float()
    
    if sr != sample_rate:
        import torchaudio.transforms as T
        wav = T.Resample(sr, sample_rate, dtype=wav.dtype)(wav)
